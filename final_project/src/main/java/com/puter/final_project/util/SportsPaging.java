package com.puter.final_project.util;
...
public class SportsPaging {
	public static String getPaging(String pageURL, int nowPage, int rowTotal, int blockList, int blockPage) {
		...
		if (isPrevPage) {
			...
			sb.append(String.format("<li><a href='sports.do?page=%d'>◀</a></li>", startPage - 1));
		} else
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (i == nowPage) { // 현재 있는 페이지
				...
				sb.append(String.format("<li><a href='sports.do?page=%d'>%d</a></li>", i, i));
			}
		} // end for
		...
		// ----------------------------------------------------------------------------------------------
		if (isNextPage) {
			sb.append(String.format("<li><a href='sports.do?page=%d'>▶</a></li>", endPage + 1));
		} else
			sb.append("<li><a href='#'>▶</a></li>");
	...
	public class SportsPagingCategory {
		public static String getPaging(String pageURL, int nowPage, int rowTotal, int blockList, int blockPage,
				String filter) {
			...
			if (isPrevPage) {
				...
				sb.append(String.format("<li><a href='sports.do?page=%d&%s'>◀</a></li>", startPage - 1, filter));
			} else
				sb.append("<li><a href='#'>◀</a></li>");
			...
			for (int i = startPage; i <= endPage; i++) {
				...
				if (i == nowPage) { // 현재 있는 페이지
					...
					sb.append(String.format("<li><a href='sports.do?page=%d&%s'>%d</a></li>", i, filter, i));
				}
			} // end for
			...
			// ----------------------------------------------------------------------------------------------
			if (isNextPage) {
				sb.append(String.format("<li><a href='sports.do?page=%d&%s'>▶</a></li>", endPage + 1, filter));
			} else
				sb.append("<li><a href='#'>▶</a></li>");
