            ...
            <script type="text/javascript">
                            ...
                            jQuery.ajax({
                                ...
                                jQuery.ajax({
                                    ...
                                    success: function (tokenResponse) {
                                        ...
                                        try {
                                            // responseText를 수동으로 JSON으로 변환
                                            var parsedResponse = JSON.parse(tokenResponse);

                                            // access_token 추출
                                            var access_token = parsedResponse && parsedResponse.response ?
                                                parsedResponse.response.access_token : null;
                                            if (access_token) {
                                                console.log("발급된 토큰:", access_token);
                                                // 이후 로직 진행
                                            } else {
                                                console.error("response 객체가 없습니다. 응답 확인 필요:", parsedResponse);
                                            }
                                        } catch (e) {
                                        ...
                                        jQuery.ajax({
                                            ...
                                            success: function (paymentResponse) {
                                                var paymentData = paymentResponse.response; // 결제 정보
                                                console.log("결제 정보:", paymentData);
                                                // 결제 정보 확인 후 처리 로직 추가
                                            },
                                            error: function (err) {
                                                console.error("결제 정보 조회 실패:", err.responseText);
                                            }
                                        });
                                    },
                                    error: function (err) {
                ...
                function updateTotalPrice() {
                    document.querySelectorAll('.product-container').forEach(item => {
                        ...
                        applyCoupon();
                        console.log("finalPrice를 계산하기 전의 price : " + price + " / quantity : " + quantity +
